...
function StoryCard({ story }) {
  return (
    <details className="group overflow-hidden rounded-4xl border border-slate-200/80 bg-white shadow-lg shadow-slate-900/5 transition duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#0F3D5E]/10">
      <summary className="cursor-pointer list-none p-6 [&::-webkit-details-marker]:hidden">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 text-sm font-bold text-[#2CB1A6]">
              {story.concern}
            </p>

            <p className="mt-4 line-clamp-3 text-[15px] font-semibold leading-7 text-slate-600">
              {story.intro}
            </p>
          </div>

          <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#F7FBFC] text-[#0F3D5E] transition group-open:rotate-180">
            <ChevronDown size={20} />
          </div>
        </div>
      </summary>
